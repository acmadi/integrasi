...
Route::get('aa.php', ['as'=>'xx',function() {
  return 'aa';
...
Route::get('json/admin', function() {
  ...
  [
    ...
    "text" => "Users",
    "iconCls"=>"icon-add",
    "route_link" => "sentinel.users.index",
    "id_datagrid" => "bidangx",
    "db" => "",
    "filter_filter" => ''
    ...
    "text" => "Groups",
    "iconCls"=>"icon-add",
    "route_link" => "sentinel.groups.index",
    "id_datagrid" => "bidangy",
    "db" => "",
    "filter_filter" => ''
    ...
    $tree = buildTree($src_array, 'parentid', 'id');
...
Route::get('json',['as' => 'json', function ()
{
  ...
  $arr=Treemenu::all();
  // dd($arr->toArray());
  $tree = buildTree($arr->toArray(), 'parentid', 'id');
  // $tree = buildTree($arr, 'parentid', 'id');
// dd($tree);
  echo json_encode(json_decode(json_encode($tree)));
  exit();
    ...
    $fnBuilder = function($siblings) use (&$fnBuilder, $grouped, $idKey) {
        foreach ($siblings as $k => $sibling) {
            ...
            if(isset($grouped[$id])) {
                ...
                $db=!empty($sibling['db'])?'/'.$sibling['db']:'';
                $sibling['text']='<a data-title="'.$sibling['db'].'"  data-link="'.$link.$db.'" style="display:block;width:200px;">'.$sibling['text'].'</a>';//turunan
                // echo $sibling['text'];
                // dd($sibling['text']);
                // print_r($sibling);
                // exit();
