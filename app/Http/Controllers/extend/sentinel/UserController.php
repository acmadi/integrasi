...
use Sentinel\Controllers\UserController as SentilenUserController;
...
class UserController extends SentilenUserController
{
    ...
    public function getUsers($value='')
    {
        // return 'ddd';
        /* supply data=======================================================-============================*/
        $users_all       = $this->userRepository->all();
        $perPage     = 10;
        $currentPage = Input::get('page') - 1;
        $pagedData   = array_slice($users_all, $currentPage * $perPage, $perPage);
        $users       = new Paginator($pagedData, $perPage, $currentPage);

        $rows=[];$i=0;
        foreach ($users as $user){
                $rows[$i]['id']=$user->hash;
                $rows[$i]['user']=$user->email;
                $rows[$i]['status']=$user->status;
    ...
    public function updateX(UserUpdateRequest $request, $hash){
        return parent::update($request,$hash);

    }
     

    public function storeX(UserCreateRequest $request)
    {
        return parent::store($request);
    }
    public function changePassword(ChangePasswordRequest $request, $hash)
    {
        return parent::changePassword($request, $hash);
    }
